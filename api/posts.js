  ...
  try {
    ...
    if (slug) {
      const rows = await sql(
        `SELECT slug, title, category, excerpt, body, viz, accent, reading_time, date, schema_markup FROM posts WHERE slug = $1 AND published = true LIMIT 1`,
        [slug]
      );
    ...
    const rows = await sql(
      `SELECT slug, title, category, excerpt, viz, accent, reading_time, date FROM posts WHERE published = true ORDER BY date DESC, id DESC`
    );
